...
export function Lesson({ title, slugName, type, availableAt }: LessonProps) {
  const { slug } = useParams<{ slug: string }>();

  const isLessonAvailable = isPast(availableAt);
  const availableFateFormatted = format(
    availableAt,
  ...
  return (
    <Link to={`/event/lesson/${slugName}`} className="group">
      <span className="text-gray-300">{availableFateFormatted}</span>
